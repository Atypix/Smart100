...
function getFallbackData(symbol: string, source_api: string, interval: string): FinancialData[] {
  ...
  try {
    ...
    const results = stmt.all({ symbol, source_api, interval }) as FinancialData[]; // Cast here
    if (results.length > 0) {
        // If we have results, we want all records from the MOST RECENT `fetched_at` timestamp available
        const mostRecentFetchedAt = results[0].fetched_at; // This should be safe if FinancialData[] is correct
        const finalResults = db.prepare(`
            ...
            ORDER BY timestamp DESC
        `).all({symbol, source_api, interval, mostRecentFetchedAt}) as FinancialData[]; // Cast here
        return finalResults;
    }
    return [];
  } catch (error) {
