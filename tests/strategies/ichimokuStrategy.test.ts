...
import { StrategyContext, StrategySignal, StrategyParameterDefinition } from '../../src/strategies/strategy.types'; // Removed HistoricalDataPoint, Portfolio, Trade
import { HistoricalDataPoint } from '../../src/services/dataService'; // Added direct import
import { Portfolio, Trade } from '../../src/backtest'; // Added direct import
import logger from '../../src/utils/logger'; // Corrected path and import type
...
describe('Ichimoku Cloud Strategy', () => {
  ...
  const defaultParams = ichimokuCloudStrategy.parameters.reduce((acc: Record<string, number | string | boolean>, p: StrategyParameterDefinition) => { // Typed acc and p
  ...
  test('should have correct id, name, and parameters defined', () => {
    ...
    expect(ichimokuCloudStrategy.parameters.find((p: StrategyParameterDefinition) => p.name === 'tenkanPeriod')?.defaultValue).toBe(9); // Typed p
  });
  ...
  test('should return HOLD if insufficient historical data for calculations', async () => { // Made async
    ...
    const signal = await Promise.resolve(ichimokuCloudStrategy.execute(mockContext)); // Added await Promise.resolve
  ...
  test('should return HOLD if calculated Ichimoku components are null', async () => { // Made async
    ...
    // A more direct way:
    const originalCalculator = jest.requireActual('../../src/strategies/implementations/ichimokuStrategy').calculateIchimokuComponents; // Corrected path
  ...
  test('should return BUY signal on strong bullish conditions', async () => { // Made async
    ...
    const buySignal = await Promise.resolve(ichimokuCloudStrategy.execute(mockContext)); // Added await Promise.resolve
